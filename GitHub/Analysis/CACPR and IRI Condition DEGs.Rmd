...
for (i in clusters) {
DefaultAssay(SO) <- "RNA"
DEGs <- FindMarkers(SO, ident.1 = "CACPR", group.by = "class.sample" , subset.ident = i, logfc.threshold = .25, min.pct = .1)
...
df <- df %>%
  kbl(caption = paste("Top 20", i, "Genes Upregulated in CACPR")) %>%
...
for (j in top5) {
  A <- VlnPlot(SO, j, idents = i, split.by = "class.sample")
  print(A)
}
}

list_objects <- ls(DEG.env)
save(list = list_objects, file = here("Output", "CACPR_All_Condition_DEGs.Rdata"), envir = DEG.env)
...
write.xlsx(alldf, sheetName = names(alldf), file = here("Output", "CACPR_All_Condition_DEGs.xlsx"))
...
for (i in clusters) {
DefaultAssay(SO) <- "RNA"
DEGs <- FindMarkers(SO, ident.1 = "IRI", group.by = "class.sample" , subset.ident = i, logfc.threshold = .25, min.pct = .1)
DEGs <- DEGs %>% rownames_to_column("gene") %>% filter(p_val < .05)
DEGs <- DEGs[order(DEGs[,"avg_log2FC"], decreasing = TRUE),]
...
list_objects <- ls(DEG.env)
save(list = list_objects, file = here("Output", "IRI_All_Condition_DEGs.Rdata"), envir = DEG.env)
...
write.xlsx(alldf, sheetName = names(alldf), file = here("Output", "IRI_All_Condition_DEGs.xlsx"))
