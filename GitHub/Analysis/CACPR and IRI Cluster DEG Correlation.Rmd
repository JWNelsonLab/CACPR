...
CACPR_list <- list(CACPR_All_PT, CACPR_All_DTL, CACPR_All_TAL, CACPR_All_DCT, 
...
names(CACPR_list) <- c("CACPR_All_PT", "CACPR_All_DTL", "CACPR_All_TAL", "CACPR_All_DCT", 
                   "CACPR_All_CNT", "CACPR_All_PC", "CACPR_All_ICA", "CACPR_All_ICB", 
                   "CACPR_All_URO", "CACPR_All_PEC", "CACPR_All_FIB", "CACPR_All_CONTRACT", 
                   "CACPR_All_PODO", "CACPR_All_EC", "CACPR_All_IMMUNE")

IRI_list <- list(IRI_All_PT, IRI_All_DTL, IRI_All_TAL, IRI_All_DCT, 
...
names(IRI_list) <- c("IRI_All_PT", "IRI_All_DTL", "IRI_All_TAL", "IRI_All_DCT", 
                   "IRI_All_CNT", "IRI_All_PC", "IRI_All_ICA", "IRI_All_ICB", 
                   "IRI_All_URO", "IRI_All_PEC", "IRI_All_FIB", "IRI_All_CONTRACT", 
                   "IRI_All_PODO", "IRI_All_EC", "IRI_All_IMMUNE")
...
for(i in 1:length(CACPR_list)){
  ...
  for(j in 1:length(IRI_list)){
    ...
    slope <- coef(model)["avg_log2FC.y"]
    sign <- if (slope < 0) {
             sign <- -1
              } else {
              sign <- 1
              }
    n_rsq <- summary(model)$r.squared * sign
...
write.csv(rsq_table, here("Output", "R2_for_IRI_Vs_CACPR_Cluster_DEGs_NO_FILTER.csv"))
...
rsq_table <- read.csv(here("Output", "R2_for_IRI_Vs_CACPR_Cluster_DEGs_No_Filter.csv"))
...
df2 <- rsq_table %>% mutate(CACPR = sub(".*\\.", "", CACPR)) %>%
                      mutate(IRI = sub(".*\\.", "", IRI))
  
  df2$CACPR <- factor(df2$CACPR, levels = unique(df2$CACPR))
  df2$IRI <- factor(df2$IRI, levels = unique(df2$IRI))
  
  
# Create the correlation dot plot
plot <- ggplot(df2, aes(x = CACPR, y = IRI, size = Shared_DEGs, color = R.squared)) +
  geom_point() +
  scale_color_gradient(low = "blue", high = "red") +
  labs(x = "X", y = "Y", title = paste("Correlation:")) +
  theme_minimal() +
  theme(panel.grid = element_blank())

# Display the correlation dot plot
print(plot)
