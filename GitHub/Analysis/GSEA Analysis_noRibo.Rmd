...
# remove rows that have the pattern "^Rp" in the "gene" column

CACPR_All_PT <- CACPR_All_PT %>% filter(!grepl("^Rp", gene))
IRI_All_PT <- IRI_All_PT %>% filter(!grepl("^Rp", gene))
...
GSEA_CACPR_All_PT
rawGSEA_IRI_All_PT
rawGSEA_CACPR_All_PT

# Save multiple objects
save(GSEA_IRI_All_PT,
GSEA_CACPR_All_PT,
rawGSEA_IRI_All_PT,
rawGSEA_CACPR_All_PT,
file = here("Output", "PT_GSEA_noRibo.RData"))
...
set1 <- GSEA_IRI_All_PT$GO_Description
set2 <- GSEA_CACPR_All_PT$GO_Description
venn <- ggvenn(list(IRI = set1, CACPR = set2))
print(venn)
...
CommonMF <- GSEAgos_common %>% group_by(Ontology_CACPR) %>% arrange_()
...
GSEAgos_common2 <- GSEAgos_common %>% 
                   dplyr::select(GO_term, GO_Description_CACPR, NES_CACPR, P.adjust_CACPR, NES_IRI, P.adjust_IRI) %>%
                   arrange(desc(NES_CACPR))

print(head(GSEAgos_common2, 10))

print(tail(GSEAgos_common2, 10))


GSEAgos_IRI_only2 <- GSEAgos_IRI_only %>% 
                   dplyr::select(GO_term, GO_Description, NES, P.adjust) %>%
                   arrange(desc(NES))

print(head(GSEAgos_IRI_only2, 10))

print(tail(GSEAgos_IRI_only2, 10))


GSEAgos_CACPR_only2 <- GSEAgos_CACPR_only %>% 
                   dplyr::select(GO_term, GO_Description, NES, P.adjust) %>%
                   arrange(desc(NES))

print(head(GSEAgos_CACPR_only2, 10))

print(tail(GSEAgos_CACPR_only2, 10))
